...
HKLM\SOFTWARE\Microsoft\Microsoft SQL Server\MSSQL15.SQL2019_CI_AS\MSSQLServer\Parameters
...
SELECT @SQLCMD = 'DBCC TRACEOFF(' + STUFF(
(
    SELECT ',' + CONVERT(VARCHAR(15), TF)
    FROM @TraceFlags
    WHERE enable = 0
    ORDER BY TF
    FOR XML PATH(''), TYPE
...
SELECT @SQLCMD = 'DBCC TRACEON(' + STUFF(
(
    SELECT ',' + CONVERT(VARCHAR(15), TF)
    FROM @TraceFlags
    WHERE enable = 1
...
DECLARE cSQLParams CURSOR LOCAL FAST_FORWARD FOR WITH cte
                                                 AS (
                                                    -- Current arguments, with new TFs added at the end. Get a row number to sort by.
                                                    SELECT *,
                                                           ROW_NUMBER() OVER (ORDER BY ISNULL(ArgNum, 999999999), TF)
                                                           - 1 AS RN
                                                    FROM @SQLArgs arg
                                                        FULL OUTER JOIN @TraceFlags tf
                                                            ON arg.Data = tf.TF2),
                                                      cte2
                                                 AS (
                                                    ...
                                                    FROM cte
                                                        ...
                                                        CROSS APPLY
                                                    (
                                                        SELECT ISNULL(Value, 'SQLArg' + CONVERT(VARCHAR(15), RN)),
                                                               ISNULL(Data, TF2)
                                                    ) ca(Value, Data)
...
BEGIN
    IF @DebugLevel = 0
        EXECUTE master.sys.xp_instance_regwrite @RegHive,
                                                @RegKey,
                                                @Value,
                                                'REG_SZ',
                                                @Data;
    RAISERROR(
                 'EXECUTE master.sys.xp_instance_regwrite ''%s'', ''%s'', ''%s'', ''REG_SZ'', ''%s''',
                 10,
                 1,
                 @RegHive,
                 @RegKey,
...
BEGIN
    ...
    IF @DebugLevel = 0
        EXECUTE master.sys.xp_instance_regdeletevalue @RegHive, @RegKey, @Value;
